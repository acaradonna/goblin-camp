...
pub fn los_visible(map: &GameMap, x0: i32, y0: i32, x1: i32, y1: i32) -> bool {
    ...
    loop {
        if let Some(i) = map.idx(x0, y0) {
            if is_opaque(map.tiles[i]) && !(x0 == x1 && y0 == y1) { return false; }
        } else { return false; }
...
pub fn compute_visibility_system(
    ...
    q: Query<(Entity, &crate::world::Position, Option<&crate::components::VisionRadius>)>,
) {
