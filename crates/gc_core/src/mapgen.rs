...
use noise::{NoiseFn, Fbm, Seedable};
    ...
    pub fn generate(&self, width: u32, height: u32) -> GameMap {
        ...
        for y in 0..height as i32 {
            for x in 0..width as i32 {
                ...
                let kind = if e < -0.2 { TileKind::Water } else if e > 0.6 { TileKind::Wall } else { TileKind::Floor };
                if let Some(i) = map.idx(x, y) { map.tiles[i] = kind; }
            }
        }
        map
    }
